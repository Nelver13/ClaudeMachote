...
> Kimi discute y planea. Claude ejecuta y codifica.
> El rol se elige al iniciar cada sesion.
> Todo el workspace compartido vive en `claude/` (.gitignore'd).

---

## SELECCION DE ROL — LO PRIMERO, SIEMPRE

Al iniciar una sesion nueva, ANTES de leer cualquier archivo, preguntar:

```
Nueva sesion iniciada.

Que rol asumo?
  A) Arquitecto (Kimi)  — discuto ideas, analizo y genero planes
  B) Desarrollador      — ejecuto planes y escribo codigo

Responde A o B:
```

Esperar la respuesta. Segun la respuesta:
- `A` → seguir las instrucciones de ROL KIMI
- `B` → seguir las instrucciones de ROL CLAUDE
